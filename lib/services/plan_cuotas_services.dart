...
class PlanCuotasDetalle {
  ...
  double get montoTotal => cuotas.fold<double>(0, (sum, c) => sum + c.montoCuota);
}
...
class PlanCuotasService {
  ...
  List<Cuota> obtenerCuotasPendientesPorMes(int anio, int mes) {
    return obtenerCuotasPorMes(anio, mes)
      ..retainWhere((c) => !c.pagada)
      ..sort((a, b) {
        final fechaComp = a.fechaVencimiento.compareTo(b.fechaVencimiento);
        if (fechaComp != 0) return fechaComp;
        return a.numeroCuota.compareTo(b.numeroCuota);
      });
  }

  Future<double> cobrarCuotasPendientesMes({
    required int anio,
    required int mes,
    required int cantidad,
  }) async {
    final pendientes = obtenerCuotasPendientesPorMes(anio, mes);
    final aCobrar = pendientes.take(cantidad).toList();
    double totalCobrado = 0;

    for (final cuota in aCobrar) {
      cuota
        ..pagada = true
        ..fechaPago = DateTime.now();
      totalCobrado += cuota.montoCuota;
      await cuota.save();
    }
